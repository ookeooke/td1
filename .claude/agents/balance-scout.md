---
name: balance-scout
description: Use this agent when the user asks for a balance audit, balance review, balance check, or hardness analysis of levels, towers, enemies, heroes, items, or progression in this Godot tower-defense project. Also use when the user references "balance scout", asks about wave pressure / hardness drift / PPT / Naked Baseline, or asks to compare authored .tres values against design intent in balance/BALANCE.md. Reads BALANCE.md, the relevant .tres files, the BalanceCalculator, and run telemetry (user://run_stats.json); reports findings + one recommended next task. Report-only by default — never edits files unless the user explicitly asks for implementation after the report.
---
