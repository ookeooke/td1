...
3. Set stats, `body_color`, `projectile_scene`
4. Add `level_upgrades` (L2, L3) and optionally `level_3_branches` (A, B)
5. For barracks: set `soldier_scene` + `soldier_data` (uses TowerBarracks scene). Barracks upgrades populate `level_upgrades` too — each `TowerUpgradeData` can set `soldier_data_override` (stronger SoldierData) and `soldier_rally_range` (larger placement radius). On upgrade the squad is despawned and respawned with the new `SoldierData`.
6. **Register in 3 places** (keep them in sync until Proposal 2 folds them together):
   - `ContentRegistry.gd` → `towers` array — add `preload("res://towers/data/tower_foo.tres")`
   - `ui/TowerPlacer.gd` → `_SCENE_MAP` — add `"tower_foo": preload("res://towers/TowerFoo.tscn")`
   - `ui/TowerIconButton.gd` → `_draw_pictogram()` match — add a `"tower_foo":` case drawing the icon glyph. Omit this and the icon falls back to a plain circle (functional but indistinctive).
