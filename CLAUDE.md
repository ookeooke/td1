...
- [docs/HERO_MULTIMODE_ARBITRATION.md](docs/HERO_MULTIMODE_ARBITRATION.md) — design-only rule for heroes that autonomously switch between two basic-attack behaviors (multi-mode). **Must be implemented + tested before any multi-mode hero is authored.** Pure-B keeps heroes single-mode by construction; dragon/trapper ship single-mode and do not need it.
